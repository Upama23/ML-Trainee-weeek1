"""Download the python file from this link. It is an implementation of an e-commerce
system where products can be added, customers can be created and then products can
...
“breakpoint()” function. Use different syntax such as where (w), next (n), step (s),
continue (c), print (p) etc.
(Hint: Negative quantity should not be allowed to add to cart and non present product
...
class ShoppingCart:
    ...
    def add_product(self, product, quantity):
        if quantity > 0:  # Make sure quantity is positive
            if product in self.products:
                self.products[product] += quantity
            else:
                self.products[product] = quantity
        else:
            print("Invalid quantity value. Quantity must be greater than 0.")
    ...
    def remove_product(self, product, quantity):
        if product in self.products:
            if quantity > 0 and self.products[product] >= quantity:
                ...
                print("Invalid quantity value or insufficient quantity in cart.")
        else:
...
"""Similar to the previous question, debug the given python file from this link. It is an
implementation of a food ordering system.
Note: Use vscode “Run and Debug” with necessary breakpoints.
(Hint: Negative quantity should not be allowed and non present food item should not be
allowed to remove)"""
...
customer.add_to_cart(food_item3, 2)  # Fixed: Ensure positive quantity

delivery_service = DeliveryService()
delivery_service.add_restaurant(restaurant1)
delivery_service.add_restaurant(restaurant2)

restaurant1.remove_from_menu(food_item2, 6)  # Fixed: Remove an appropriate quantity
restaurant2.remove_from_menu(food_item1, 1)  # Fixed: Remove an appropriate quantity
